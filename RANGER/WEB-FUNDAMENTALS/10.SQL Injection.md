...
If you've received this error message confirms the existence of an SQL Injection vulnerability.
...
**Out-of-Band (OOB) SQL Injection** is a more advanced technique used when the application does not return data directly to the screen (**In-Band**) and the response is not stable enough to use timing or boolean checks (**Blind**).

Instead of trying to see the data in the website's response, the attacker forces the database to send the data to an **external server** that the attacker controls.
