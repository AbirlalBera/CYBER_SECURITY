...
**Error-Based SQLi:** Relies on the database printing error messages directly to the screen. These errors reveal the database structure (tables, columns, and syntax).

**Union-Based SQLi:** Uses the `UNION` operator to combine the results of the original query with a new query created by the attacker.
