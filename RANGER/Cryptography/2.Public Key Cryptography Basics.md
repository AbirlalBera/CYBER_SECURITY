...
- Solves the **key-sharing problem** in symmetric encryption
- Allows two parties to create a **shared secret over an insecure channel**
- No pre-shared secret required
- Eavesdroppers can see all exchanged values but **cannot compute the secret**
...
Alice computes:
```A = g^a mod p A = 3^13 mod 29 A = 19
```

Bob computes:

`B = g^b mod p B = 3^15 mod 29 B = 26`
...
`s = B^a mod p s = 26^13 mod 29 s = 10`

Bob computes:

`s = A^b mod p s = 19^15 mod 29 s = 10`
